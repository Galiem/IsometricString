    ...
    if (r % 2 == 0){
        if (j-i == r/2){
            ...
            if (f[r+i] == f[r+j]){
                ...
                if (j < n)
                {
                    INT lmin = SOLONmin ( invSA[ i ], invSA[ j ] );
                    INT rmax = SOLONmax ( invSA[ i ], invSA[ j ] );

                    cout << "lmin: " << lmin <<" rmax: "<< rmax <<endl;
                    if( LCP[rmq(lmin+1, rmax)] >= r/2){
                        condr3 = true;
...
INT witnessesConstructor( char * f, INT l, INT * errpos, INT nErrpos, string * u, string * v, INT * LCP, INT * invSA, rmq_succinct_sct<> rmq){
    ...
    if (nErrpos == 1){
        ...
        cout<< "falfa: " <<(falfa1)<<endl;
        cout<< "fbeta: " <<(fbeta1)<<endl;
        cout<< "falfa: " <<((falfa1[i] - '0') - 1) % 4 << " i: " << i<< endl;
        if(falfa1[i] != '0'){
...
INT twoErrorOverlaps(unsigned char * f, INT * twoeolens, INT * nTwoeolens, INT ** allerrpos,INT * totalErrors, INT * nAllerrpos, INT * LCP, INT * invSA, rmq_succinct_sct<> rmq){
    ...
    *nTwoeolens = 0;
    //allerrpos = (INT *) malloc(n * sizeof(INT));
    *nAllerrpos = 0;
    *totalErrors = 0;
    ...
    for (INT i = 1; i < n; i++){
        ...
        while (d <= 2){
            ...
            if (i+l < n)
            {
                ...
                cout << "LCA: " << LCP[rmq(lmin+1, rmax)] <<endl;
            }
            ...
            if (d == 2 && l == n - i){
                cout<< "ho trovato un two error" << endl;
                twoeolens[*nTwoeolens] = l;
                cout<< "twoeolens[" << *nTwoeolens << "]: " << twoeolens[*nTwoeolens] << endl;
                *(nTwoeolens) = *nTwoeolens + 1;
                std::copy(allerrpostmp, allerrpostmp + nAllerrpostmp, allerrpos[*totalErrors]);
                cout<< "allerrpos[0]: " << allerrpos[0] << endl;
...
int main(int argc, char const *argv[]){
    ...
    cout<< "twoeolens: " << twoeolens[0] <<endl;
